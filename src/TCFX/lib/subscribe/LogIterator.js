...
class LogIterator extends LazyPromise {
  ...
  async _isConfirmed(epochNumber, threshold) {
    ...
    const risk = await this.cfx.getConfirmationRiskByHash(
      lodash.last(blockHashArray),
    );
  ...
  async _readConfirmed({
    ...
    return loop({delta, timeout}, async () => {
      const logs = await this.cfx.getLogs({
        ...
        limit:
          this.filter.limit === undefined
            ? undefined
            : this.filter.limit - this._count,
      });
