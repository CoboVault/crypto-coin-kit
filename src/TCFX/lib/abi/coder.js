...
function _pack(coders, array) {
  ...
  lodash.zip(coders, array).forEach(([coder, value]) => {
...
function _unpack(coders, stream) {
  ...
  lodash.zip(coders, array).forEach(([coder, value], index) => {
...
class IntegerCoder extends Coder {
  ...
  constructor({name, type, signed = false, bits = 256} = {}) {
    assert(
      Number.isInteger(bits) && 0 < bits && bits <= 256 && bits % 8 === 0,
      {
    ...
    this.signed = signed;
    this.size = bits / 8;
    this.bound = JSBI.leftShift(
      JSBI.BigInt(1),
      JSBI.BigInt(bits - (this.signed ? 1 : 0)),
    );
  }
  ...
  encode(value) {
    ...
    assert(
      JSBI.LE(JSBI.BigInt(0), twosComplement) &&
        JSBI.LT(twosComplement, this.bound),
      {
        message: 'bound error',
        expect: `0<= && <${this.bound}`,
