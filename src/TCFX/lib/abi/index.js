...
function formatFullName({name, inputs}) {
  return `${name}(${inputs
    .map(
      param =>
        `${getCoder(param).type} ${param.indexed ? 'indexed ' : ''}${
          param.name
        }`,
    )
    .join(', ')})`;
...
class EventCoder {
  ...
  constructor({anonymous, name, inputs = []} = {}) {
    ...
    this.inputs = inputs;
    this.dataCoder = getCoder({
      type: 'tuple',
      components: inputs.filter(component => !component.indexed),
    });

    this.NamedTuple = namedTuple(
      ...inputs.map((input, index) => input.name || `${index}`),
    );
  }
    ...
    this.inputs.forEach((component, index) => {
      if (component.indexed) {
        ...
        topics.push(
          value === null
            ? null
            : format.hex(getCoder(component).encodeIndex(value)),
        );
      }
    });
