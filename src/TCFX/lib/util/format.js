 ...
 */
format.uInt = Parser(toNumber).validate(
 ...
 * > format.riskNumber('0xe666666666666666666666666666666666666666666666666666666666666665')
 0.9
 */
format.riskNumber = format.bigUInt
  .parse(v => Number(Big(v).div(MAX_UINT_256)))
  .or(null);
 ...
 * > format.buffer(3.14)
 Error("not match hex")
 */
format.buffer = Parser(v =>
  Buffer.isBuffer(v) ? v : Buffer.from(format.hex(v).substring(2), 'hex'),
);
