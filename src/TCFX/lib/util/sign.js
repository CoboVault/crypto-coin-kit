...
function randomPrivateKey(entropy = randomBuffer(32)) {
  if (!(Buffer.isBuffer(entropy) && entropy.length === 32)) {
    throw new Error(
      `entropy must be 32 length Buffer, got "${typeof entropy}"`,
    );
  }
...
function publicKeyToAddress(publicKey) {
  const buffer = Buffer.from(keccak256(publicKey), 'hex').slice(-20);
...
function decrypt(
  ...
  return crypto
    .createDecipheriv(algorithm, derived.slice(0, 16), iv)
    .update(cipher);
}
