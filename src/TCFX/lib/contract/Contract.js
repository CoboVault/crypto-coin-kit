...
class Contract {
  ...
  constructor(cfx, {abi, address, bytecode}) {
    ...
    this.constructor = new ContractConstructor(
      cfx,
      this,
      lodash.first(abiTable.constructor),
      bytecode,
    );
    ...
    const methodArray = lodash.map(
      abiTable.function,
      fragment => new ContractMethod(cfx, this, fragment),
    );
    const eventArray = lodash.map(
      abiTable.event,
      fragment => new ContractEvent(cfx, this, fragment),
    );
