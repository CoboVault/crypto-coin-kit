...
class Called {
  ...
  async estimateGasAndCollateral(options) {
    try {
      return await this.cfx.estimateGasAndCollateral({
        to: this.to,
        data: this.data,
        ...options,
      });
  ...
  async call(options, epochNumber) {
    try {
      const hex = await this.cfx.call(
        {to: this.to, data: this.data, ...options},
        epochNumber,
      );
...
class ContractMethodOverride {
  ...
  call(...args) {
    ...
    if (!acceptArray.length) {
      throw new Error(
        `can not match override "${rejectArray.join(
          '|',
        )}" with args (${args.join(',')})`,
      );
    }
    if (acceptArray.length > 1) {
      throw new Error(
        `can not determine override "${acceptArray.join(
          '|',
        )}" with args (${args.join(',')})`,
      );
    }
