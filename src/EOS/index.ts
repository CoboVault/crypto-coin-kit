...
export class EOS implements Coin {
    private chainId = 'aca376f206b8fc25a6ed44dbdc66547c36c6c33e3a119ffbeaef943642f0e906';
    ...
    public generateTransaction = async (txData: TxData, signer: SignProvider) => {
        const eosInstance = this.getEosInstance(txData.header, this.getSignProvider(signer));
        const tx = await eosInstance.transfer(
            txData.data.from,
            txData.data.to,
    ...
    public generateTransactionSync = (txData: TxData, signer: SignProviderSync) => {
        ...
        const rawTx = Object.assign({}, defaultHeaders, this.formatHeaders(txData.header));
        // @ts-ignore
        rawTx.actions = [{
            account: 'eosio.token',
    ...
    private getEosInstance = (txHeader: Header, eosSignProvider: any) => {
        ...
        return EosInstance({
            ...
            transactionHeaders: (expireInSeconds: number, callback: any) => callback(null, headers),
            broadcast: false,
            sign: true,
    ...
    private toEOSAmount = (amount: number, symbol = 'EOS') => {
    ...
    private getExpiration = (txHeader: Header) => {
        ...
        return new Date(txHeader.time + expireInSeconds * 1000).toISOString().split('.')[0]
    };

    private buildSignature(sig: Result) {
        const r = Buffer.from(sig.r, 'hex');
        const s = Buffer.from(sig.s, 'hex');
        return Signature.fromBuffer(Buffer.concat([Buffer.alloc(1, sig.recId + 4 + 27), r, s])).toString();
    }
