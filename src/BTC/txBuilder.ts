...
import { Destination, NonWitnessUtxo, TxData, TxInputItem, TxOutputItem, WitnessUtxo } from './index'
...
export default class PsbtBuilder {
    ...
    public addOutputForPsbt = (txData: TxData) => {
        if(this.isDestinationOutputs(txData.outputs)) {
            ...
            const totalInputs = txData.inputs.reduce((acc: number, cur: TxInputItem) => acc + cur.utxo.value, 0)
            const changeAmount = totalInputs - txData.outputs.amount - txData.outputs.fee
            this.psbt.addOutput({
    ...
    private isDestinationOutputs = (out: TxOutputItem[] | Destination): out is Destination => {
