...
describe('Coin.IOST', () => {
  const privKey =
    '632790825e1d1234b8a72a493ae08f3a8328c1faf5dfcbee2d838c6cc9f854ae';
  const pubKey =
    '36cbcb409c374786439aa6907a04e41dbc3402abc95c9234909cd21b0496c243';
  const iost = new IOST();
  ...
  it('should valid pubKey', () => {
    const valid = iost.isAddressValid(
      '4guFd1weqq2Nvfxt6EDSfztUjp44trRzfJMqXumJ3qRt',
    );
  ...
  it('should generate right tx', async () => {
    ...
    const tx = await iost.generateTransaction(
      txData,
      Ed25519KeyProvider(privKey, pubKey),
    );
  ...
  it('should generate right tx sync', () => {
    const tx = iost.generateTransactionSync(
      txData,
      Ed25519KeyProviderSync(privKey, pubKey),
    );
  });
});
