...
} from '../../EOS/signProvider';

const privKey =
  '2fa110edcefe9d64ccaece846245cebbe46893f16f4d22e68ef49a7c8b6e4874';
const pubKey =
  '03c218e7c1dc1749d161af204334acffa9d384337a37f92cf03502a33a64fe3789';
...
describe('Coin.EOS', () => {
  ...
  it('should generate right dress', () => {
    ...
    expect(address).toBe(
      'EOS8JiV4sKPmMPoTCrGV8dhm4foKSBRdnyRw3FWzf6mQz6H8KXNMR',
    );
  });
  ...
  it('should valid address', () => {
    expect(
      eos.isAddressValid(
        'EOS86B1XBB2B7u3QcdM5tuuTdRMLFfVDzsTcjHvDqcFq78MSstUxp',
      ),
    ).toBe(true);
  });
  ...
  it('should generate tx', async () => {
    const eosTx = await eos.generateTransaction(
      txData,
      SignProviderWithPrivateKey(privKey),
    );
    expect(eosTx.txId).toBe(
      '7af10fb78976604e080b0fe34c523e5ec722b800f4a9b1af350d6238039560dd',
    );
    const tx = JSON.parse(eosTx.txHex);
    expect(tx.signatures[0]).toBe(
      'SIG_K1_Kf8yNx2UXUgqxuCMWzaune51QEE7BPungQiVcKx8dKtfhVPkc5SCjB4fPrUbqi2avEzioG6da3NR19j77dG3wmGaX3ssNS',
    );
  });
  ...
  it('should generate tx sync', () => {
    const eosTx = eos.generateTransactionSync(
      txData,
  ...
  it('should generate nut token tx', async () => {
    ...
    expect(eosTx.txId).toBe(
      '350849c86f988659a83a0509d02a37a729feb10eb1f16c3e84f25d691f4a49b9',
    );
    const tx = JSON.parse(eosTx.txHex);
    expect(tx.signatures[0]).toBe(
      'SIG_K1_JvuGPLCqxJWjoULXVVftEZJ8BT5VWkjh4o1RgpeFegRMiMG25akDHn8TNoPZ6VPTzwSz3iEeJ7k4QaCLBK6ERE9oJAHprR',
    );
  });
